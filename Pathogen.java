...
public class Pathogen
{
	...
	{
		Pathogen.solveMaze(maze);
		return paths;
	}

	// Returns a double representing the rating of difficulty I find this assignment.
	public static double difficultyRating()
	{
		return 3.5;
	}

	// Returns a double representing the hours pent on this assignment.
	                                 ...
	                                 int height, int width)
	{
		...
		{
			newRow = currentRow + moves[i][0];
			newCol = currentCol + moves[i][1];

			// Check if move is in bounds, not a wall, and not marked as visited.
			...
			// Building the string of directions taken during maze search.
			if (route.length() != 0)
			{
				route.append(' ');
			}
			...
			visited[newRow][newCol] = SPACE; // Undo visited position.
			maze[currentRow][currentCol] = PERSON;

			// Removing the previous location from the route during backtracking.
			if (route.length() >= 2)
			{
			...
			// Prints the maze when a move gets undone (which is effectively another kind of move).
			if (Pathogen.animationEnabled)
			{
	...
	// that position before, not a wall, nor coronavirus, nor an out of bounds position).
	private static boolean isLegalMove(char [][] maze, char [][] visited,
	                                   int row, int col, int height, int width)
	{
		if (maze[row][col] == WALL || visited[row][col] == BREADCRUMB || 
			maze[row][col] == CORONAVIRUS || col <= 0 || col >= width -1 || 
			row <= 0 || row >= height - 1)
			return false;
